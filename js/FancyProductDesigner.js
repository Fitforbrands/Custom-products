...
export default class FancyProductDesigner extends EventTarget {
  ...
  constructor(elem, opts = {}) {
    ...
    if (
      elem.classList.contains("fpd-off-canvas") ||
      elem.classList.contains("fpd-topbar")
    )
      this.mainOptions.toolbarPlacement = "smart";

    if (
      Array.isArray(this.mainOptions.pricingRules) &&
      this.mainOptions.pricingRules.length
    ) {
    ...
    this.translator.loadLangJSON(
      this.mainOptions.langJSON,
      this.#langLoaded.bind(this)
    );
  }
  ...
  #ready() {
    ...
    if (this.mainOptions.keyboardControl) {
      addEvents(document, "keydown", (evt) => {
        if (this.currentViewInstance && this.currentViewInstance.fabricCanvas) {
          ...
          if (
            currentElement &&
            !["TEXTAREA", "INPUT"].includes(targetNodename)
          ) {
            switch (evt.which) {
              case 8:
                //remove element
    ...
    window.addEventListener("resize", (evt) => {
      ...
      if (
        this.currentElement &&
        !["INPUT", "TEXTAREA"].includes(document.activeElement)
      ) {
    ...
    addEvents(this, ["productCreate", "modalDesignerOpen"], (evt) => {
      ...
      this.firstProductCreated =
        this.mainOptions.modalMode && evt.type === "modalDesignerOpen";
    });
    ...
    addEvents(
      ...
      ["mouseup", "touchend"],
      (evt) => {
        let fpdContainers = Array.from(
          document.querySelectorAll(".fpd-container")
        );
    ...
    if (this.mainOptions.dragDropImagesToUploadZones) {
      ...
      addEvents(document.body, ["mousedown", "touchstart"], (evt) => {
        ...
        if (evt.which == 1) {
          ...
          if (target.classList.contains("fpd-draggable")) {
            ...
            this.draggedPlaceholder.className =
              "fpd-dragged-image fpd-shadow-1 fpd-hidden";
      ...
      addEvents(document.body, ["mousemove"], (evt) => {
        if (this.draggedPlaceholder) {
          this.draggedPlaceholder.style.left =
            evt.pageX - targetGridItem.offsetWidth * 0.5 + "px";
          this.draggedPlaceholder.style.top =
            evt.pageY - targetGridItem.offsetHeight * 0.5 + "px";
      ...
      addEvents(document.body, ["mouseup"], (evt) => {
        ...
        if (
          !this.loadingCustomImage &&
          targetGridItem &&
          this.mouseOverCanvas
        ) {
          this._addGridItemToCanvas(
            targetGridItem,
            this.mouseOverCanvas.uploadZone
              ? { _addToUZ: this.mouseOverCanvas.title }
              : {}
          );
        }
        ...
        if (!viewInstance.fabricCanvas.getElementByTitle(fpdElement.title)) {
          ...
          if (fpdElement.getType() === "text") {
            propertyKeys = propertyKeys.concat(
              Object.keys(this.mainOptions.textParameters)
            );
          } else {
            propertyKeys = propertyKeys.concat(
              Object.keys(this.mainOptions.imageParameters)
            );
          }
  ...
  selectProduct(index, categoryIndex) {
    ...
    this.currentCategoryIndex =
      categoryIndex === undefined ? this.currentCategoryIndex : categoryIndex;
  ...
  addView(view) {
    ...
    FancyProductDesignerView.relevantOptions.forEach((key) => {
      ...
      relevantMainOptions[key] = isPlainObject(mainProp)
        ? { ...mainProp }
        : mainProp;
    });

    view.options = isPlainObject(view.options)
      ? deepMerge(relevantMainOptions, view.options)
      : relevantMainOptions;
    ...
    if (
      this.viewInstances.length == 0 &&
      view.options.industry &&
      view.options.industry.type
    ) {
    ...
    viewInstance.fabricCanvas.on({
      ...
      beforeElementAdd: (opts) => {
        ...
        if (
          this.mainBar &&
          this.uiManager &&
          this.uiManager.currentLayout == "small"
        ) {
      ...
      elementAdd: ({ element }) => {
        ...
        if (
          ...
          element.replace &&
          element.replaceInAllViews
        ) {
          this.viewInstances.forEach((viewInst, i) => {
            if (this.currentViewIndex != i) {
              const replacedElem = viewInst.fabricCanvas.getElementByReplace(
                element.replace
              );
        ...
        if (
          ...
          var viewIndex = this.getViewIndexByWrapper(
            viewInstance.fabricCanvas.wrapperEl
          );
          ...
          if (this.colorLinkGroups.hasOwnProperty(element.colorLinkGroup)) {
            ...
            if (typeof element.colors === "object") {
              ...
              const colorGroupColors = this.mainOptions
                .replaceColorsInColorGroup
                ? element.colors
                : this.colorLinkGroups[element.colorLinkGroup].colors.concat(
                    element.colors
                  );
              this.colorLinkGroups[element.colorLinkGroup].colors =
                arrayUnique(colorGroupColors);
        ...
        if (
          this.productCreated &&
          this.mainOptions.hideDialogOnAdd &&
          this.mainBar
        ) {
          this.mainBar.toggleContentDisplay(false);
        }
          ...
          if (group && group.elements) {
            group.elements.forEach((groupElem) => {
              if (element.id != groupElem.id) {
                ...
                const targetElem = targetView.fabricCanvas.getElementByID(
                  groupElem.id
                );
    ...
    viewInstance.fabricCanvas.on("sizeUpdate", ({ canvasHeight }) => {
      ...
      if (mainBarClasslist.contains("fpd-sidebar")) {
        ...
        if (canvasHeight != this.mainWrapper.container.offsetHeight)
          this.mainBar.container.style.height =
            this.mainWrapper.container.offsetHeight + "px";
      }
    });

    if (viewInstance.names_numbers && viewInstance.names_numbers.length > 1) {
      viewInstance.changePrice(
        (viewInstance.names_numbers.length - 1) *
          viewInstance.options.namesNumbersEntryPrice,
        "+"
      );
    }
  ...
  #updateElementTooltip() {
    ...
    if (
      ...
      if (element.isOut && element.boundingBoxMode === "inside") {
        ...
        let displayText =
          displaySize.width + "x" + displaySize.height + displaySize.unit;
      ...
      if (this.mainTooltip.classList.contains("fpd-show")) {
        ...
        const viewStageRect =
          this.currentViewInstance.fabricCanvas.wrapperEl.getBoundingClientRect();

        let leftPos =
          viewStageRect.left + oCoords.mt.x - this.mainTooltip.clientWidth / 2;
        let topPos =
          viewStageRect.top + oCoords.mt.y - this.mainTooltip.clientHeight - 20;
  ...
  #setWarnings() {
    ...
    if (this.productCreated && element) {
      ...
      if (
        element.isCustomImage &&
        dpi !== null &&
        dpi < this.mainOptions.customImageParameters.minDPI
      ) {
        ...
        if (
          this.mainOptions.aiService.serverURL &&
          this.mainOptions.aiService.superRes
        ) {
          const superResBtn = document.createElement("span");
          superResBtn.className = "fpd-btn";
          superResBtn.innerText = this.translator.getTranslation(
            "misc",
            "ai_upscale_btn"
          );
          ...
          addEvents(superResBtn, "click", (evt) => {
            ...
            const toPx = parseInt(
              (this.mainOptions.customImageParameters.minDPI *
                displaySize.width) /
                lin
            );
            ...
            this.toggleSpinner(
              true,
              this.translator.getTranslation("misc", "loading_image")
            );
  ...
  calcElementDPI(element) {
    if (
      ...
      !element.uploadZone &&
      element.isBitmap() &&
      objectHasKeys(this.currentViewInstance.options.output, [
        "width",
        "height",
      ]) &&
      objectHasKeys(this.currentViewInstance.options.printingBox, [
        "left",
        "top",
        "width",
        "height",
  ...
  calcDisplaySize(element) {
    ...
    if (
      ...
      if (unit != "px") {
        ...
        widthRatio =
          this.currentViewInstance.options.output.width /
          this.currentViewInstance.options.printingBox.width;
        heightRatio =
      ...
      this.viewInstances.forEach((viewInst) => {
        viewInst.fabricCanvas.getObjects().forEach((fabricObj) => {
          if (
            ...
            const linkedPropKeys = Object.keys(element).filter((key) =>
              textLinkGroupProps.includes(key)
            );
  ...
  getViewIndexByWrapper(wrapperEl) {
    return Array.from(
      this.productStage.querySelectorAll(".fpd-view-stage")
    ).indexOf(wrapperEl);
  }
  ...
  selectView(index = 0) {
    ...
    toggleElemClasses(
      document.body.querySelectorAll(
        '.fpd-btn[data-action="previous-view"], .fpd-btn[data-action="next-view"]'
      ),
      ["fpd-hidden"],
    ...
    toggleElemClasses(
      document.body.querySelectorAll(
        '.fpd-btn[data-action="previous-view"], .fpd-view-prev'
      ),
      ["fpd-disabled"],
      this.currentViewIndex == 0
    );
    ...
    toggleElemClasses(
      document.body.querySelectorAll(
        '.fpd-btn[data-action="next-view"], .fpd-view-next'
      ),
      ["fpd-disabled"],
      this.currentViewIndex === this.viewInstances.length - 1
    );
    ...
    this.currentViewInstance.fabricCanvas.enableRuler =
      this.mainOptions.rulerFixed;
    ...
    if (viewIndex === -1) {
      for (var i = 0; i < this.viewInstances.length; ++i) {
        allElements = allElements.concat(
          this.viewInstances[i].fabricCanvas.getElements(
            elementType,
            deselectElement
          )
        );
      ...
      if (this.viewInstances[viewIndex]) {
        allElements = this.viewInstances[viewIndex].fabricCanvas.getElements(
          elementType,
          deselectElement
        );
      } else {
    ...
    image.onload = () => {
      ...
      let imageH = image.height,
        ...
        currentCustomImageParameters =
          this.currentViewInstance.options.customImageParameters;
  ...
  _downloadRemoteImage(source, title, options = {}, callback = null) {
    if (!this.mainOptions.fileServerURL) {
      alert(
        "You need to set the fileServerURL in the option, otherwise file uploading does not work!"
      );
      return;
    }
  ...
  toggleResponsive(toggle) {
    toggle =
      toggle === undefined
        ? this.container.classList.contains("fpd-not-responsive")
        : toggle;
  ...
  getProduct(onlyEditableElements = false, customizationRequired = false) {
    let customizationChecker = false,
      jsMethod =
        this.mainOptions.customizationRequiredRule == "all" ? "every" : "some";
    ...
    if (customizationRequired && !customizationChecker) {
      Snackbar(
        this.translator.getTranslation("misc", "customization_required_info")
      );
      return false;
    }
    ...
    this.getElements().forEach((element) => {
      if (
        element.isOut &&
        element.boundingBoxMode === "inside" &&
        !element.__editorMode
      ) {
        Snackbar(
          element.title +
            ": " +
            this.translator.getTranslation("misc", "out_of_bounding_box")
        );
  ...
  getProductDataURL(callback = () => {}, options = {}, viewRange = []) {
    options.onlyExportable =
      options.onlyExportable === undefined ? false : options.onlyExportable;
    options.enableRetinaScaling =
      options.enableRetinaScaling === undefined
        ? false
        : options.enableRetinaScaling;
    ...
    let printCanvas = new fabric.Canvas(hiddenCanvas, {
      ...
      targetViews =
        viewRange.length == 2
          ? this.viewInstances.slice(viewRange[0], viewRange[1])
          : this.viewInstances;
    ...
    const _addCanvasImage = (viewInst) => {
          ...
          (img) => {
            ...
            if (viewCount > 0) {
              ...
              printCanvas.setDimensions({
                height:
                  printCanvas.getHeight() +
                  viewInst.options.stageHeight * multiplier,
              });
            }
      ...
      if (viewInst.options.stageWidth * multiplier > printCanvas.getWidth()) {
        printCanvas.setDimensions({
          width: viewInst.options.stageWidth * multiplier,
        });
      }
    };
      ...
      for (var i = 0; i < dataURLs.length; ++i) {
        ...
        image.onload = () => {
          ...
          if (imageLoop == dataURLs.length) {
            ...
            popupBlockerAlert(
              popup,
              this.translator.getTranslation("misc", "popup_blocker_alert")
            );
      ...
      if (element.getType() === "text") {
        if (_usedFonts.indexOf(element.fontFamily) === -1) {
          var fontObj = { name: element.fontFamily },
            result = this.mainOptions.fonts.find(
              (e) => e.name == element.fontFamily
            );
  ...
  getOrder(options = {}) {
    ...
    this.getUsedColors().forEach((hexValue) => {
      let colorName =
          this.mainOptions.hexNames[hexValue.replace("#", "").toLowerCase()],
  ...
  getFileServerURL() {
    ...
    if (
      objectGet(this.viewInstances[0].options, "industry.type") == "engraving"
    ) {
